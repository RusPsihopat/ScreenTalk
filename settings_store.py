...
DEFAULTS = {
    ...
    "autostart_admin": False,  # автозапуск (если включён) — с правами администратора
}
...
def save_fields(**fields):
    """Обновляет и сохраняет ТОЛЬКО перечисленные поля, не трогая остальные.

    Нужно для вещей, которые должны сохраняться сразу же (положение окна,
    режим закрепления, автозапуск) — независимо от кнопки "Сохранить" в
    основной панели настроек, и не затирая её несохранённые изменения.
    """
    current = load()
